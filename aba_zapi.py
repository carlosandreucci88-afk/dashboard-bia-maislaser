...
def tela_zapi_aguardando_validacao():
    ...
    # não passa por aqui em rerun dentro do fragment scope). Full rerun = df
    # fresco do Supabase = overrides de modo viram stale. Ex: coord clicou AUTO,
...
# ============================================================================
# v10.10 (Refactor UX, 23/07/2026): @st.experimental_fragment isola cada card
# do rerun global. Motivo: sem fragment, clicar Validar/MANUAL/AUTO dispara
# st.rerun() que recarrega tela inteira (~5-6s do Streamlit re-executar todos
# os cards + métricas + config recepção). Com fragment, só o card do clique
# re-renderiza (<1s).
#
# ⚠️ IMPORTANTE — usar @st.experimental_fragment (não @st.fragment):
# Streamlit 1.35.0 em produção (requirements.txt). @st.fragment nativo só
# chegou em 1.36. Idem: st.rerun() dentro de fragment em 1.29-1.36 já reroda
# só o fragment por default (parâmetro scope="fragment" só existe em 1.37+).
#
...
def _render_acao_manual(camp_id, tel, nome, bia_puxou_dt):
    ...
    if st.session_state.get(f"confirm_pending_{camp_id}"):
        ...
        if cancelar:
            ...
            # v10.10: st.rerun() dentro de fragment em Streamlit 1.29-1.36
            # reroda SÓ o fragment (sem parâmetro scope, que é 1.37+).
            st.rerun()
        ...
        if confirmar:
            ...
            if resp.get("_erro") or resp.get("erro"):
                ...
                _zapi_get_validacao_supabase.clear()
                st.balloons()
            # v10.10: st.rerun() dentro do fragment reroda só o card (1.29-1.36).
...
def _executar_set_modo(tel, modo, nome, camp_id=None):
    ...
    # v10.10: se camp_id foi passado (fluxo dentro do fragment do card),
    # marca card_modo_override_ (transiente, limpo no full rerun natural)
    # e faz rerun. st.rerun() dentro de fragment em Streamlit 1.29-1.36
    # reroda SÓ o fragment por default.
    # Se camp_id não foi passado (retrocompat), st.rerun() faz full rerun
    # porque está fora de contexto fragment.
